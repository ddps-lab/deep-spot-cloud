...
def lambda_handler(event, context):
    ...
    conn = pymysql.connect(host="xxxxx", user="mj", passwd="xxx",
    ...
    conn = pymysql.connect(host="xxxxxx", user="mj", passwd="xxxx",
                           db="migration", connect_timeout=5)
    with conn.cursor() as cur:

        cur.execute("UPDATE Route SET price=%s WHERE id=%s", (p_price, int(p_id)))
