...
def start_new_instance(checkpoint_path, step, random_id):
    ...
    c = pycurl.Curl()
    c.setopt(c.URL,
             'https://xxxxxxx.execute-api.us-west-2.amazonaws.com/deploy/start-new-instance?ud=%22' + ud + '%22')
    c.perform()
...
def uploading_current_status_to_rds(step):
    ...
    c = pycurl.Curl()
    c.setopt(c.URL,
             'https://xxxxxx.execute-api.us-west-2.amazonaws.com/deploy/upload-deeplearning-status-to-rds?az=%22' + az + '%22&step=%22' + str(
